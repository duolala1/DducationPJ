...
if __name__=="__main__":
    ...
    # dictionary = [u'信息论概念']
    global All_Book
    # dictionary = [u'信息论概念',u'信息理论与信息系统',]
    dictionary = [u'信息论概念',
                   u'信息理论与信息系统',
                   u'控制论',
                   u'系统论',
    ...
    for i in range(len(dictionary)):
        ...
        for book in books:
            cur = conn.execute('SELECT count(*) from model_bookinfo')
            num = 0
            for row in cur:
                num = row[0]
            bookID = num + 1
            conn.execute("INSERT INTO model_bookinfo (bookID,bookName,bookDetail,imgUrl,author,bookIntro ) VALUES (?,?,?,?,?,?)",(bookID,book['book_title'],book['content'],book['img_url'],book['author'],book['introduction']))
            # for label in book['label']:
            #     conn.execute("INSERT INTO model_labels (bookID,label) VALUES (?,?)",(book['bookID'],label))
            conn.execute("INSERT INTO model_labels (bookID,label) VALUES (?,?)",(bookID,dictionary[i]))
            conn.commit()
            print 'bookID',bookID
        print 'label',i,'saved'
        All_Book = []
